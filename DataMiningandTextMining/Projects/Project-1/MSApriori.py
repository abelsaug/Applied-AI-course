...
def ms_apriori(transactions, mis, sdc):
    ...
    while F[k - 1]:
        ...
        if k == 1:
            ...
            C.append(MSCandidateGen(F[k - 1], sdc, sup_count, mis, N))
        print "C:", C
...
def level2CandidateGen(L, sdc, sup_count, N, mis):
    ...
    for i in L:
        if float(sup_count[i]) / N >= mis[i]:
            ...
            postL = L[indexOfI + 1:]
            for h in postL:
                # print tuple([i, h]), float(sup_count[h]) / N >= mis[i], (abs(float(sup_count[h]) - float(sup_count[i])) <= sdc), sup_count[h], sup_count[i], sdc
                if float(sup_count[h]) / N >= mis[i] and abs(float(sup_count[h])/N - float(sup_count[i])/N) <= sdc:
                    C2.append(tuple([i, h]))
    return C2


def MSCandidateGen(Fk1, sdc, sup_count, mis, N):
    Ck = []
    Fk = []
    for f1 in Fk1:
        ...
        for f2 in postF1:
            if f1[:-1] == f2[:-1]:
                if f1 and f2 and f1[-1] < f2[-1] and abs(float(sup_count[f1[-1]])/N - float(sup_count[f2[-1]])/N) <= sdc:
                    c = f1
                    c.append(f2[-1])
                    Ck.append(c)
